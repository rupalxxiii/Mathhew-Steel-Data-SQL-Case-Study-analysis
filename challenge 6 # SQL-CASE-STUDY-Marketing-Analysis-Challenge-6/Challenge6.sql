...
select M.campaign_name ,count(T.transaction_id) as No_of_transactions
...
select P.product_id ,P.product_name , sum(T.quantity) as highest_sales_quantity
...
select M.campaign_id, M.campaign_name, concat("$",round(sum(S.price*T.quantity),2)) as Total_Revenue
from marketing_campaigns as M join sustainable_clothing as S using(product_id) 
join transactions as T using(product_id)
...
select S.category, concat("$", round(sum(S.price*T.quantity),2)) as Total_Revenue
...
select S.product_name, sum(T.quantity) as higher_quantity
...
select M.campaign_name , concat("$", round(sum(S.price*T.quantity)/DATEDIFF(end_date,start_date),2))
as revenue_perday
from marketing_campaigns as M join sustainable_clothing as S using (product_id)
join transactions as T using(product_id)
...
(
  select S.product_name,
		 S.product_id,
         round(sum(S.price*T.quantity)) as Total
         from transactions as T
         join sustainable_clothing as S using(product_id)
         group by S.product_name, S.product_id
         )
	select product_id, product_name, concat(round(Total*100/(select sum(Total) from contribution),2),"%") as
	percentage from contribution order by product_id ASC ;


-- 8. COMPARE THE AVERAGE QUANTITY SOLD DURING MARKETING CAMPAIGNS TO OUTSIDE THE MARKETING CAMPAIGNS?

select
avg(Case when product_id in (select distinct product_id from marketing_campaigns) 
         then quantity else null end) as avg_quantity_during_campaigns,
avg(Case when product_id not in (select distinct product_id from marketing_campaigns) 
         then quantity else null end) as avg_quantity_outside_campaigns
from transactions;
...
select
round(sum(case when t.product_id in (select distinct product_id from marketing_campaigns) then t.quantity*s.price else 0 end), 2) 
as revenue_during_campaigns,
round(sum(case when t.product_id not in (select distinct product_id from marketing_campaigns) then t.quantity*s.price else 0 end),2)
as revenue_outside_campaigns 
from transactions as t join sustainable_clothing as s using (product_id);
...
select * ,dense_rank() over(order by average_quantity asc) as rnk
from ranking;
